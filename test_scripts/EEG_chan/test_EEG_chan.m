...
% 281 - 13 groups*2 = 255 channels
length(unique(ch)) % 253 unique values
[v, w] = unique( ch, 'stable' )
...
% leave 176 channels remove 85 channels
ch_bad=[241 242 243 238 239 240 ...
    ...
    249 254 73 93 104 113 122 135 147 157 167 176 189 201 218 227 231 ...
    230 226 225 219 190 177 168 158 148 136 123 114 105 94 67 253 252 248 244 241 257];
% remove 48 channels
ch_bad=[241 242 243 238 239 240 ...
    244 245 246 247 251 256 91 102 111 120 133 145 165 174 187 199 208 216 229 233 237 236 235 234 ...
...
for c=1:256
    if ismember(c, labeled_ch)
        plot3(x(c),y(c),z(c),'.','MarkerSize',10,'color',darkgreen);
        text(x(c),y(c),z(c), ch_labels{c},'color',darkgreen,'FontSize', 12);
    elseif ismember(c, ch_bad)
        plot3(x(c),y(c),z(c),'.','MarkerSize',10,'color',matlab_red);
        text(x(c),y(c),z(c), num2str(c),'color',matlab_red,'FontSize', 12);
    else
        plot3(x(c),y(c),z(c),'.','MarkerSize',10,'color',darkgreen);
        text(x(c),y(c),z(c), num2str(c),'color',darkgreen,'FontSize', 12);      
    end
end
%% Color Scheme
% Plots - color scheme
